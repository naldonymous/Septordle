...
export default function Game() {
  ...
  const { guesses, currGuess, setCurrGuess, gameOver, message } = useGameLogic(solution);
  ...
  return (
    <main className="flex flex-col items-center gap-10 relative">
      ...
      <Keyboard onKey={(key: string) => handleKeyPress({ key } as unknown as KeyboardEvent, currGuess, setCurrGuess, setGuesses, setGameOver, setMessage, solution, guesses)} />
    </main>
  );
}